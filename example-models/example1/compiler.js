...
function reverseTopo(nodes, edges) {
  ...
  edges.forEach(e => {
    g[e.to.id].push(e.from.id);
    indeg[e.from.id]++;
  });

  // 排序依據：GlobalVariable 最優先，APIEndpoint[USE]其次，其他最晚
  const priority = id => {
    const n = nm[id];
    if (n.type === 'GlobalVariable') return 0;
    if (n.type === 'function') return 1;
    if (n.type === 'APIEndpoint[USE]') return 2;
    return 3;
  };
  ...
  while (q.length) {
    ...
    q.sort((a, b) => priority(a) - priority(b));
  }
...
function findDuplicateNodeIds(fullGraph) {
  const seen = new Set();
...
function compile(graphPath = 'graph.json', outDir = 'prompt-txt') {
  ...
  if(findDuplicateNodeIds(fullGraph)){
    throw new Error('❌ Compile Error: Detect duplicate node ID!');
    return;
  }
