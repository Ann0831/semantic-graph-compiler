...
const returnMainpage = require('./returnMainpage.js');
...
const post_register_handler = async (req, res) => {
  try {
    ...
    const requestedHashed_Password = req.body.requestedHashed_Password;
...
檢查req中的cookie，如果有username和Token，就送去給sqldatacenter中的Verify_LoginToken驗證，如果成功就運用sqldatacenter.searchuserIDbyusername和username來找到user_id，以及在req.query找到post_id，用sqldatacenter.diduserlikepost及user_id和post_id，如果回傳true則res.send("true")，其他情況則res.send("false");
...
const get_userlikepost_handler = async (req, res) => {
  ...
  const liked = await sqldatacenter.diduserlikepost(user_id, parseInt(post_id));
  if (liked) {
    res.send("true");
  } else {
    res.send("false");
