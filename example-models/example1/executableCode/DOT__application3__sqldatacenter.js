...
function generatePostId(timestamp, userId) {
  ...
  const idStr = timestamp.toString() + userIdStr;
  return BigInt(idStr);
}
...
logic:
都用transaction
先檢查去userlikesinfo檢查這個user_id是不是已經對此post_id按讚了。若無，直接return "neverLikeBefore"；若有，在userlikesinfo刪除這個like，然後再去userpostsinfo更新userpostsinfo中的該post_id的讚數。這個步驟中，成功就return "success"， 失敗就return "fail"）。
...
async function unsendlike(user_id, post_id) {
  ...
  try {
    ...
    const [likeCountResult] = await connection.query(
      'SELECT COUNT(*) AS count FROM userlikesinfo WHERE post_id = ?',
      [post_id]
    );

    const newCount = likeCountResult[0].count;

    // Update like_count in userpostsinfo
    await connection.query(
...
logic:
一切都用transaction。
先檢查去userlikesinfo檢查這個user_id是不是已經對此post_id按讚了。若有，直接return "alreadyExist"；若無，userlikesinfo插入一個這個like，然後再去userpostsinfo更新userpostsinfo中的該post_id的讚數。（這個步驟中，成功就return "success"， 失敗就return "fail"）。
...
async function Write_LoginToken(username) {
  try {
    ...
    const payload = { username, iat: issuedAt };
    const token = jwt.sign(payload, JWT_SECRET, { expiresIn });

    await pool.query(
      `INSERT INTO userLoginToken (username, token, issued_at, expires_at, created_at)
       VALUES (?, ?, CURRENT_TIMESTAMP, FROM_UNIXTIME(?), CURRENT_TIMESTAMP)
       ON DUPLICATE KEY UPDATE token = VALUES(token), issued_at = CURRENT_TIMESTAMP, expires_at = FROM_UNIXTIME(?), created_at = CURRENT_TIMESTAMP`,
      [username, token, issuedAt + expiresIn, issuedAt + expiresIn]
    );
...
async function updateAndSendGlobalRecommendNumber() {
  try {
    // Step 1: Get post_id, like_count, secondTime of non-deleted posts
    const [posts] = await pool.query(
      'SELECT post_id, like_count, secondTime FROM userpostsinfo WHERE is_deleted = 0'
    );

    const now = Date.now();

    // Step 2: Compute globalrecommendscore and update database
    for (const post of posts) {
      const timeSincePosted = now - Number(post.secondTime);
    ...
    // Step 3: Return post_id, globalrecommendscore, user_id
    const [result] = await pool.query(
      'SELECT post_id, globalrecommendscore, user_id FROM userpostsinfo WHERE is_deleted = 0'
    );
